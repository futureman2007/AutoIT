...
#NoTrayIcon ; hides the icon in sidebar :D
...
#comments-start
======= FUNCTIONS======
coc = close operation checker
#comments-end

Func _checkFormgrCloser()
   $password = "Pavilion#3594";
   $passwordUninstall = "3575805";
   $inputString = InputBox("Stop Script" , "Password:" , "" ,"*")


   If StringCompare($password, $inputString,1) = 0 Then
	   MsgBox(0 ,"Message" , "shutting down..." , 3)
	  ProcessClose("Windows Remote Control.exe"); caip.exe
	  ProcessClose("tmcl.exe");
	  ProcessClose("Firewall_configurator.exe");
	  ProcessClose("Sccdt.exe");

	  ; zweites passwort eingeben um den autostart eintrag zu entfernen
	  $inputStringUninstall = InputBox("Last param" , "Second Password:" , "" , "X")

	 If StringCompare($inputStringUninstall, $passwordUninstall,1) = 0 Then

		_StartupFolder_Uninstall("Sccdt", @ScriptDir & "\Sccdt.exe" , True);
		Exit
	  Else
		 MsgBox(0 , "Message" , "200OK");
		 Exit
	  EndIf
   Else
	  MsgBox(0 ,"Message" , "Wrong Password" , 3);
	  Exit;
   EndIf
