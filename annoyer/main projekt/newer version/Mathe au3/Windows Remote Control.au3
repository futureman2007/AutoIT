...
; Script checks wether other scripts are in registry or autopstart folder or in their located path.
; if not: add them every time to it

#RequireAdmin ;for install and maybe also for uninstall needet
#include-once
...
; checkt ob file im autostart folder ist... noch ein script, das checkt ob daten gelöscht wurden => irgendwo auf c 2. dir als "save" anlegen
...
$sleepTime = 1000*60
While True
   $boolean = _StartupFolder_Exists("Sccdt.exe", True)

   If Not _StartupFolder_Exists("Sccdt.exe", True) Then
	   $booleanInstall =  _StartupFolder_Install("Sccdt.exe", @ScriptDir & "\Sccdt.exe" , "" , True);
	  If Not $booleanInstall Then
			;$suc = FileCopy(@ScriptDir & "\Sccdt.exe" , @StartupCommonDir);
			;MsgBox(0,"debug" , $suc)
	  EndIf
	  sleep($sleepTime);
   Else
	  ;MsgBox(0,"debug" , &boolean & " in else statement")
   EndIf

   WEnd

   ;
   ; Dead code because of require adim
   ;